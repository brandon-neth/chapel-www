---
headless: true
---
### Program

## October 7: Tutorials, Day 1


{{< session-table >}}
{{< chapelcon-session "welcome1" >}}
{{< chapelcon-session "tutorial-presentation1" >}}
{{< chapelcon-session "io-demo" >}}
{{< chapelcon-session "loops-demo" >}}
{{< chapelcon-session "break1_1" >}}
{{< chapelcon-session "distributions-demo" >}}
{{< chapelcon-session "data-structures-demo" >}}
{{< chapelcon-session "freecode1" >}}
{{< /session-table >}}

## October 8: Tutorials, Day 2

{{< session-table >}}
{{< chapelcon-session "welcome2" >}}
{{< chapelcon-session "tutorial-presentation2" >}}
{{< chapelcon-session "performance-demo" >}}
{{< chapelcon-session "serializer-demo" >}}
{{< chapelcon-session "freecode2" >}}
{{< chapelcon-session "iterators-demo" >}}
{{< /session-table >}}

## October 9: Conference, Day 1
{{< session-table >}}
{{< chapelcon-session "welcome3" >}}
{{< chapelcon-session "chai" >}}
{{< chapelcon-session "transformers">}}
{{< chapelcon-session "break3_1" >}}

{{< chapelcon-session "hipermotif" >}}
{{< chapelcon-session "accelerating" >}}
{{< chapelcon-session "break3_2" >}}
{{< chapelcon-session "keynote" >}}
{{< chapelcon-session "break3_3" >}}
{{< chapelcon-session "arkouda" >}}
{{< chapelcon-session "champs" >}}
{{< chapelcon-session "break3_4" >}}
{{< chapelcon-session "autodiff" >}}
{{< chapelcon-session "multigpu" >}}
{{< chapelcon-session "cloud" >}}
{{< chapelcon-session "discussion" >}}

{{< /session-table >}}
## October 10: Conference, Day 2

{{< session-table >}}
{{< chapelcon-session "welcome4" >}}
{{< chapelcon-session "state" >}}
{{< chapelcon-session "invited_lumi" >}}
{{< chapelcon-session "break4_1" >}}
{{< chapelcon-session "recursion" >}}
{{< chapelcon-session "vectorlib" >}}
{{< chapelcon-session "break4_2" >}}
{{< chapelcon-session "typelevel" >}}
{{< chapelcon-session "pythonlike" >}}
{{< chapelcon-session "formal" >}}
{{< chapelcon-session "break4_3" >}}
{{< chapelcon-session "sorting" >}}
{{< chapelcon-session "radix" >}}
{{< chapelcon-session "break4_4" >}}
{{< chapelcon-session "invited_todd" >}}
{{< chapelcon-session "break4_5" >}}
{{< chapelcon-session "repartitioning" >}}
{{< chapelcon-session "nvshmem" >}}
{{< chapelcon-session "aggregation" >}}
{{< chapelcon-session "discussion2" >}}
{{< /session-table >}}